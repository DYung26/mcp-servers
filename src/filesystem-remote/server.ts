...
// Create a server factory that mirrors the existing filesystem MCP server but
// does NOT start a stdio transport. This allows external transports (HTTP/SSE)
// to connect to the server instance.
export const createServer = (initialDirectories: string[] = []): ServerFactoryResponse => {
  // Start with provided directories (from command-line) or empty array
  // These can be updated via MCP Roots protocol if client supports it
  let allowedDirectories: string[] = [...initialDirectories];
  ...
  const server = new McpServer(
    {
      name: "secure-filesystem-server-remote",
      version: "0.2.0",
    }
  );
  ...
  server.server.oninitialized = async () => {
    ...
    if (clientCapabilities?.roots) {
      ...
      // Client doesn't support roots - use command-line directories
      if (allowedDirectories.length > 0) {
        console.error("Client does not support MCP Roots, using allowed directories from server configuration:", allowedDirectories);
      } else {
        console.error("Warning: No allowed directories available. Client does not support MCP Roots and no directories were provided at startup. All filesystem operations will fail.");
      }
    }
  };
  ...
  return {
    ...
    cleanup: (sessionId?: string) => {
      // No-op cleanup for now; resource cleanup can be added if needed.
    }
  };
};
